...
describe("RegisterPage", () => {
  it("shows a loading state while the session is being verified", () => {
    ...
    expect(
      screen.getByText("로그인 정보를 확인하는 중..."),
    ).toBeInTheDocument();
  });
  ...
  it("shows validation errors when submitting an empty form", async () => {
    ...
    await user.click(
      await screen.findByRole("button", { name: "등록 신청서 제출하기" }),
    );
  ...
  it("submits successfully and shows a loading state", async () => {
    mswServer.use(
      http.post("/api/companies", async () => {
        await new Promise((resolve) => setTimeout(resolve, 50));
        return HttpResponse.json(
          {
            data: {
              company: { ...FIXTURE_COMPANIES[0], id: "new-company", name: "새 조경" },
            },
          },
          { status: 201 },
        );
    ...
    await user.type(await screen.findByLabelText(/^업체명/), "새 조경");
